...
from horoscopes.Client import HoroscopeClient, HoroscopeClient
...
class GCFTest(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        self.horoscope.sentiment = self._gclient.get_sentiment(self.horoscope.content)
        self.horoscope.entities = self._gclient.get_entities(self.horoscope.content)
    ...
    def test_local_update(self):
        ...
        target_valence = self._pu._score_to_valence(self.horoscope.sentiment.score)
        target_energy = self._pu._magnitude_to_energy(self.horoscope.sentiment.magnitude)
        targets = {
            'valence': target_valence,
            'energy': target_valence
        }
        entities = [{"name": entity.name} for entity in self.horoscope.entities]
