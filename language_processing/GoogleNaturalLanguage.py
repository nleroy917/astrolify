...
class LanguageClient:
    ...
    Requires a google cloud platform account, project, and an API add on
    to be configured. In addition, the google authntication credentials
    for this service must be in the directory and an environment variable
    should be set pointing to these credentials.
    ...
    def _create_document(
            self, text, type_=language_v1.Document.Type.PLAIN_TEXT,
            language='en'):
        """
        Create a document object to analyze with the client

        :param text: - the text of the document - should be plain-text
        :param type_: the content type object taken from the language
                    processing library. Available types: PLAIN_TEXT, HTML
        :param language: the language of the text.
                -> Optional. If not specified, the language is automatically
                   detected. For list of supported languages:
